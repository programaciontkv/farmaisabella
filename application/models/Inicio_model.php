...
$resultado=$this->db->query($query);
    ...
    public function opc_facturar($rol_id){
    	 
    	$this->db->select("rp.opc_id as op ");
    	$this->db->from(" erp_roles_opcion rp, erp_opciones eo");
    	$this->db->where("eo.opc_nombre like 'FACTURA%' and rp.rol_id ='$rol_id' and eo.opc_id =rp.opc_id ",null);
    	$this->db->order_by(" rp.opc_id",'asc');
    	$this->db->limit("1");
    	$resultado=$this->db->get();
    	return $resultado->row();
    }
